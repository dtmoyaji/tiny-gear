 ...
 * @author dtmoyaji
 */
public class SelectableGenericTableEditView extends AbstractView {
    ...
    public SelectableGenericTableEditView(GearApplication app) {
        super(app);
    }
    ...
    public void redraw() {
        ...
        this.tableSelector = new DataTableView("tableSelector", this.getTable(TableInfo.class)) {
            ...
            public void onRowClicked(AjaxRequestTarget target, KeyValueList modelObject) {
                SelectableGenericTableEditView.this.onTableSelectorRowClicked(target, modelObject);
            }
        ...
        int defaultRowsPerPage = Integer.parseInt(
                this.getGearApplication().getSystemVariable(
                        SelectableGenericTableEditView.KEY_DEFAULT_ROWS_PER_PAGE,
                        "2"
                )
        );
        ...
        boolean allowDelete = Boolean.parseBoolean(
                this.getGearApplication().getSystemVariable(
                        SelectableGenericTableEditView.KEY_DEFAULT_ALLOW_DELETE,
                        "false"
                )
        );
        ...
        defaultRowsPerPage = Integer.parseInt(
                this.getGearApplication().getSystemVariable(
                        SelectableGenericTableEditView.KEY_DEFAULT_TABLESELECTOR_ROWCOUNT,
                        "2"
                )
        );
    ...
    private void onTableSelectorRowClicked(AjaxRequestTarget target, KeyValueList modelObject) {
        ...
        try (ResultSet rs = tinfo.select(tinfo.TablePhisicalName.sameValueOf(tableName))) {
            ...
            Logger.getLogger(SelectableGenericTableEditView.class.getName()).log(Level.SEVERE, null, ex);
        }
